...
from NetCDF_generator import save_velocities_to_netCDF, generate_regrided
...
plt.savefig(results_directory+'/'+str(int(tmin/time_steps_per_day))+'_IC.png')

##########################################################################################################################################################
Fmap, DFDt, time_adv_mod = trajectory_advection(IC, time_data, Interpolant_u, Interpolant_v, lon_grid, lat_grid, timemod, dt, Ncores)

np.save(results_directory+'/'+str(int(tmin/time_steps_per_day))+'_Fmap_matrix.npy', Fmap)
np.save(results_directory+'/'+str(int(tmin/time_steps_per_day))+'_advection_time.npy', time_adv_mod)
...
unique_labels = dict(zip(labels, handles))
ax.legend(unique_labels.values(), unique_labels.keys(), loc="upper center", ncol=3, bbox_to_anchor=(0.5, 1.15), fancybox=True)
plt.savefig(results_directory+'/'+str(int(tmin/time_steps_per_day))+'_Advected_trajectories.png')
