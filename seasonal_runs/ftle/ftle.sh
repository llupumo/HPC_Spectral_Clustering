...
# Loop over years and seasons
results_directory="${directory}/Fmap/${FTLE_params}" 


Fmap_file_check="${results_directory}/${season}_${year}_FTLE.npy"
...
if [[ -f "$Fmap_file_check" ]]; then
    ...
    time srun python /cluster/home/llpui9007/Programs/FTLE/FTLEIce_seasons.py \
        "$Ncores" \
        "$dt" \
        "$dx" \
        "$dy" \
