...
def integration_dFdt_melting(time, x, X, Y, Interpolant_u, Interpolant_v, periodic, bool_unsteady, time_data, verbose = False, linear = False, timemod=1):
    '''
    Wrapper for RK4_step(). Advances the flow field given by u, v velocities, starting from given initial conditions. 
    The initial conditions can be specified as an array. 
...
def RK4_step(t, x1, dt, X, Y, Interpolant_u, Interpolant_v, periodic, bool_unsteady, time_data, linear = False):
    ...
    # Update position at the third midpoint.
    x3 = x1 + .5 * k2
    
    # Update time
    t = t0+.5*dt
    
    # Compute x_prime at the third midpoint.
...
def parallel_Fmap(x0_batch, y0_batch, time_adv, lon_grid, lat_grid, Interpolant_u, Interpolant_v, periodic, bool_unsteady, time_data, timemod, verbose = False, linear=False, melting=True):
    
    # Initial conditions
    X0 = np.array([x0_batch, y0_batch]) # array (2, Nx*Ny)
    if melting==True:
        I = integration_dFdt_melting(time_adv, X0, lon_grid, lat_grid, Interpolant_u, Interpolant_v, periodic, bool_unsteady, time_data, verbose = False, linear=False, timemod=timemod) # array (Nt, 2, Nx*Ny)
